@extends('layouts.app')
@section('title', 'Kelola Data Peminjaman Ruangan')
@section('content')
<div class="content">
...
<div class="modal fade" id="verifikasi" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formAdd" method="POST" action="{{ route('admin.keloladata.peminjamanruangan.verifikasi')}}" enctype="multipart/form-data" validate>
                    @csrf
...
<div class="modal fade" id="verifikasis" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formAdd" method="POST" action="{{ route('admin.keloladata.peminjamanruangan.verifikasis')}}" enctype="multipart/form-data" validate>
                    @csrf
                    <div class="form-group">
                        <label>Nama</label>
                        <input type="hidden" name="tgl_transaksi" value="<?php echo date('d-m-Y');?>"/>
...
<form method="post" enctype="multipart/form-data" class="form-horizontal" id="formhapus" action="{{ route('admin.keloladata.peminjamanruangan.hapus')}}">
   @csrf
   <input type="hidden" name="id" id="idhapus" value="">
</form>

<form method="post" enctype="multipart/form-data" class="form-horizontal" id="formhapuss" action="{{ route('admin.keloladata.peminjamanruangan.hapuss')}}">
   @csrf
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".verifikasi", function (e) {
          var id = $(this).data("id");
          var url = '{{ route("admin.keloladata.peminjamanruangan.detail", ":id") }}';
        ...
        $(document).on("click", ".verifikasis", function (e) {
          var id = $(this).data("id");
          var url = '{{ route("admin.keloladata.peminjamanruangan.details", ":id") }}';
          url = url.replace(':id', id);
          $.ajax({
              type: 'GET',
              url: url,
    ...
    $(function() {
        var table = $('#example2').DataTable({
           ...
           ajax: {
           url: '{!! route('admin.keloladata.peminjamanruangan.data') !!}',
           },
           columns: [
        ...
        var tables = $('#example3').DataTable({
           ...
           ajax: {
           url: '{!! route('admin.keloladata.peminjamanruangan.datas') !!}',
           },
           columns: [
               { data : 'DT_RowIndex', name : 'id' },
               { data : 'nama', name : 'nama'},
