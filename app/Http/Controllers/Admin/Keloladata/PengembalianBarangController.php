...
class PengembalianBarangController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.keloladata.pengembalianbarang.index',compact(['jenis','barang']));
    }
    ...
    public function datas(Request $request)
    {
        ...
        if ($request->jenisxx !='') 
        {
            $datas = $datas->where('jenis_barang_id', 'like', '%' . $request->jenisxxx . '%');
        }
        return Datatables::of($datas)
    ...
    public function verifikasi(Request $req)
    {
        ...
        if ($validator->fails()) {
            ...
            { 
                //Menambah stok
                $barangs = Barang::find($itemsx->barang_id);
                $barangs->jumlah_stok = $barangs->jumlah_stok+$itemsx->jumlah;
                $barangs->save();
            ...
            try{
                foreach ($barangall as $key => $value) {
                    ...
                    $text29 = 'Pengembalian Terverifikasi, Barang Silahkan Di Kembalikan.';
                    $text3 = 'Nama : '.$req->nama;
                    $text4 = 'NIM : '.$req->nim;
        ...
        return redirect()->route('admin.keloladata.pengembalianbarang.index')->with('pesan', 'Data Berhasil Di Verifikasi !');
    }
        ...
        if ($validator->fails()) {
            ...
            try{
                foreach ($barangall as $key => $value) {
                    ...
                    $text41 = 'Lama Peminjaman : '.$datas->lama_pinjam.' Hari';
                    $text5 = 'Tanggal Peminjaman : '.$req->tgl_transaksi;
                    $text51 = 'Tanggal Pengembalian: '.Helper::indonesian_date($datas->tgl_kembali,'j F Y');
                    $text22 = 'Barang :';
                    $text6 = str_replace(array('_', '-', '.'), chr(10), $barangall[$key]);
                    foreach ($text6 as $values){
                       $string .=  '- '.$values;
    ...
    public function detail($id)
    {
        ...
        if (isset($data['transaksi']['catatan_kmbl'])) {
            $data['transaksi']['catatan_kmbl'] = $data['transaksi']['catatan_kmbl'];
        }else{
            $data['transaksi']['catatan_kmbl'] = '-';
        }
