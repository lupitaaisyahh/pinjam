...
class PeminjamanBarangController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.keloladata.peminjamanbarang.index',compact(['jenis','barang']));
    }
        ...
        ->addColumn('aksi', function ($data)
        {
            return '
                <center>
                        ...
                        <a href="#" data-id="'.$data->id.'" title="Hapus" rel="tooltip" class="btn btn-icon btn-round btn-danger btn-sm hapusbtns" data-placement="top" title="" data-original-title="Hapus">
                            <i class="fas fa-trash fa-sm"></i>
                        </a>
                </center>
            ';
    ...
    public function verifikasi(Request $req)
    {
        ...
        if ($validator->fails()) {
            ...
            $datas->status_op = $req->status;
            $datas->tgl_verifikasi_op = $req->tgl_verifikasi_op;

            if ($datas->status_op == 1) {
                $datas->status_pj = 1;
                $datas->status_wd = 1;
            }else{
                $datas->status_pj = 4;
            ...
            try{
                foreach ($barangall as $key => $value) {
                    ...
                    if ($datas->status_op == 1) {
                        ...
                        $text23 = 'Verifikasi Operator : Terverifikasi';
                        $text26 = 'Tanggal Verifikasi : '.$datas->tgl_verifikasi_op;
                        $text29 = 'Peminjaman Terverifikasi, Barang Silahkan Di Ambil.';
                    }else{
                        ...
                        $text26 = 'Tanggal Verifikasi : '.$datas->tgl_verifikasi_op;
                        $text29 = 'Peminjaman Tidak Terverifikasi.';
                    }
                    $text3 = 'Nama : '.$req->nama;
                    $text4 = 'NIM : '.$req->nim;
        ...
        return redirect()->route('admin.keloladata.peminjamanbarang.index')->with('pesan', 'Data Berhasil Di Verifikasi !');
    }
        ...
        if ($validator->fails()) {
            ...
            try{
                foreach ($barangall as $key => $value) {
                    ...
                    $text7 = 'Catatan : ';
                    $text8 = $datas->catatan;
                    $chat_id = Helper::ambilDosen($datas->dosen_id,'chat_id');
                    $data = $text1 . "\n" . "\n" . $text2. "\n" . $text24. "\n" . $text25. "\n" . $text23. "\n" . $text26. "\n" . $text3. "\n" . $text41. "\n" . $text5. "\n" . $text22. "\n" . $string. "\n" . $text7. "\n" . $text8. "\n" . $text29;
                }
    ...
    public function hapus(Request $req)
    {
        ...
        $peminjaman->delete();
        return redirect()->route('admin.keloladata.peminjamanbarang.index')->with('pesan', 'Data Berhasil Di Hapus!');
    }

    public function hapuss(Request $req)
    {
        $id = $req->id;
        $peminjaman = PeminjamanBarang::find($id);
        $items = PeminjamanBarangItems::where('peminjaman_barang_id', $peminjaman->id)->get();
